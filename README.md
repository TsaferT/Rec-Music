# Разработка программы для составления музыкальньных рекомендаций контента пользователю

## Описание

Данный проект представляет собой систему рекомендаций, которая помогает пользователям находить наиболее релевантный контент на основе их предпочтений и поведения. Система использует современные алгоритмы машинного обучения и обработки данных для анализа взаимодействий пользователей с контентом и создания персонализированных рекомендаций. Программа ориентирована на улучшение пользовательского опыта и повышение вовлеченности за счет точных и индивидуальных предложений контента.

## Авторы

- [Александра Бондаренко](https://github.com/sashalieb)
- [Дмитрий Зебзеев](https://github.com/TsaferT)
- [Кирилл Сексяев](https://github.com/sexyaev)

![](documentation/cat.gif)

## Макет дизайна сайта 🎵

Добро пожаловать на страницу подбора музыки! Вот визуальный макет, который демонстрирует интерфейс и шаги, которые проходят пользователи для создания персонализированной музыкальной подборки.

### 1. Главная страница
На главной странице пользователю предлагается начать подбор музыки, всего одним кликом.

<p align="center">
  <img src="documentation/Frame1.png" alt="Главная страница" width="70%">
</p>

### 2. Выбор жанров и исполнителей
Далее пользователю предоставляется возможность выбрать интересующие его жанры и любимых исполнителей, чтобы подборка была максимально точной.

<p align="center">
  <img src="documentation/Frame2.png" alt="Выбор жанров" width="70%">
</p>

<p align="center">
  <img src="documentation/Frame3.png" alt="Выбор исполнителей" width="70%">
</p>

### 3. Готовая подборка
После выбора всех предпочтений пользователь получает свою уникальную подборку музыки, созданную специально для него.

<p align="center">
  <img src="documentation/Frame4.png" alt="Готовая подборка" width="70%">
</p>

> Этот макет поможет вам получить представление о том, как работает подбор музыки на нашем сайте и какой интерфейс увидит каждый пользователь.

